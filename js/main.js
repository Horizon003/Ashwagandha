  ...
  const onScroll = () => {
    if (!ticking) {
      requestAnimationFrame(() => {
        ...
        heroImg.style.transform = `scale(${scale}) translateY(${progress * 30}px)`;
        ticking = false;
      });
      ticking = true;
    }
...
function initViewer360() {
  ...
  function startAuto() {
    ...
    autoInterval = setInterval(() => setFrame(currentFrame + 1), 120);
    if (autoBtn) {
      autoBtn.classList.add('active');
      autoBtn.innerHTML = '<i class="fa-solid fa-pause"></i> Auto Rotating';
    }
  }
  ...
  function stopAuto() {
    ...
    if (autoBtn) {
      ...
      autoBtn.innerHTML = '<i class="fa-solid fa-play"></i> Auto Rotate';
    }
  }
...
function initImageFallbacks() {
  ...
  if (heroImg) {
    heroImg.addEventListener('error', () => {
      const bg = heroImg.closest('.hero-bg');
